...
export function AddToolDialog({
	...
	const [settings, setSettings] = useState<{ [key: string]: string }>(
		Object.keys(integrations[tool].settings).reduce((acc, key) => {
			acc[key] = ''
			return acc
	...
	return (
		<Dialog onOpenChange={(isOpen) => !isOpen && setTool('')} open={!!tool}>
			<DialogContent className="sm:max-w-xl">
				<DialogHeader>
					<DialogTitle>Add tool: {integrations[tool].name}</DialogTitle>
					<DialogDescription>{integrations[tool].description}</DialogDescription>
				</DialogHeader>
				<div className="flex flex-col space-y-2">
					{Object.entries(integrations[tool].settings).map(([key]) => (
